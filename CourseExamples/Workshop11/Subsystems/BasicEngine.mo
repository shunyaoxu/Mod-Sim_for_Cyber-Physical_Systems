...
model BasicEngine
  ...
  Modelica.Blocks.Math.Gain gain(k=k) annotation (Placement(transformation(extent={{-8,-10},{12,10}})));
  Modelica.Mechanics.Rotational.Sources.Torque torque annotation (Placement(transformation(extent={{42,-10},{62,10}})));
  parameter Real k=300 "Gain value multiplied with input signal" annotation (Dialog(tab="max_torque"));
equation
